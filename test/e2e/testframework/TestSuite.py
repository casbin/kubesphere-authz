...
class TestSuite:
    ...
    def setUp(self)->None:
        ...
        self.logFileHandler=open("%s/test/e2e/testlog/%s"%(self.workspacePath,logFileName),"w")
        cmd=[
            "%s/test/e2e/testbuild/main.exe"%(self.workspacePath),
            self.configFilePath,
        ]
        self.webhookProcess=subprocess.Popen(cmd, cwd=self.workspacePath,stdout=self.logFileHandler,stderr=self.logFileHandler)
    ...
    def test(self)->Tuple[int,int]:
        ...
        for i in range(0,len(self.testCases)):
            ...
            if webhookRunning!=None:
                ...
                print("[E2E Test]:FAILED WEBHOOK HAS CRASHED. Test suit %s, Test case %s"%(self.name,os.path.basename(yamlFilePath)))
                fail+=1
                continue
            cmd=[
                "minikube",
            ...
            res=subprocess.Popen(cmd,stdout=self.logFileHandler,stderr=self.logFileHandler)
            res.wait()
            if (shouldSuccess and res.returncode==0) or (not shouldSuccess and res.returncode!=0):
                # passed
                print("[E2E Test]:PASSED Test suit %s, Test case %s"%(self.name,os.path.basename(yamlFilePath)))
                success+=1
            else:
                #failed
                print("[E2E Test]:FAILED Test suit %s, Test case %s"%(success,os.path.basename(yamlFilePath)))
                fail+=1
        return (success,fail)
