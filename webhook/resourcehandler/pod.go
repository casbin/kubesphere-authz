...
func CheckTrustedImageOfPod(review v1.AdmissionReview, model string, policy string) error {
	enforcer, err := casbin.NewEnforcer(model, policy)
	if err != nil {
		return err
	}
	var resourceKind = review.Request.Resource.Resource
	if resourceKind != "pods" {
		//not deployment resources, approve
		return nil
	}
